/*
  # Enhanced Coaching Trainer System Schema
...
-- Add columns to existing training_sessions table
DO $$
BEGIN
...
-- Add columns to existing client_assignments table
DO $$
BEGIN
  -- Add assignment_type
  IF NOT EXISTS (
...
INSERT INTO workout_templates (name, description, category, estimated_duration_minutes, created_by, is_public) VALUES
  ('Beginner Full Body', 'A complete full body workout for beginners', 'Full Body', 45, '22222222-2222-2222-2222-222222222222', true),
  ('Upper Body Strength', 'Focus on building upper body strength', 'Upper Body', 60, '22222222-2222-2222-2222-222222222222', true),
  ('HIIT Cardio Blast', 'High intensity interval training for cardio', 'HIIT', 30, '22222222-2222-2222-2222-222222222222', true),
  ('Lower Body Power', 'Build lower body strength and power', 'Lower Body', 50, '22222222-2222-2222-2222-222222222222', true)
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  FIRST_VALUE(cp.weight_kg) OVER (PARTITION BY cp.client_id ORDER BY cp.date DESC) as current_weight,
  FIRST_VALUE(cp.weight_kg) OVER (PARTITION BY cp.client_id ORDER BY cp.date ASC) as starting_weight
FROM client_progress cp
JOIN profiles c ON c.id = cp.client_id
LEFT JOIN profiles t ON t.id = cp.trainer_id
GROUP BY cp.client_id, c.full_name, cp.trainer_id, t.full_name, cp.weight_kg, cp.date;
...
DO $$
BEGIN
  RAISE NOTICE 'Enhanced Coaching Trainer System schema has been successfully created!';
  RAISE NOTICE 'New tables: exercises, workout_templates, template_exercises, workout_plans, plan_sessions, client_progress, client_goals, conversations, messages';
  RAISE NOTICE 'Enhanced tables: training_sessions, client_assignments';
  RAISE NOTICE 'Views created: trainer_dashboard_view, client_progress_summary';
