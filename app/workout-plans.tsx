...
import { ArrowLeft, Plus, Search, Filter, Calendar, Users, Clock, Edit, Trash2, Copy, X, ChevronRight } from 'lucide-react-native';
import { useColorScheme, getColors } from '@/hooks/useColorScheme';
import { router } from 'expo-router';
import { WorkoutPlan, Client } from '@/types/workout';
import { getPlans, getClients } from '@/utils/storage';
...
export default function WorkoutPlansScreen() {
  ...
  const filterPlans = () => {
    ...
    if (searchQuery.trim()) {
      filtered = filtered.filter(plan =>
        ...
        getClientName(plan.clientId).toLowerCase().includes(searchQuery.toLowerCase())
      );
    }
  ...
  const renderPlanCard = (plan: WorkoutPlan) => {
    ...
    const duration = getDuration(plan.startDate, plan.endDate);
        ...
        <View style={styles.planMeta}>
          ...
          <View style={styles.metaItem}>
            ...
            <Text style={styles.metaText}>
              {Object.values(plan.schedule).filter(Boolean).length} workouts/week
            </Text>
          </View>
        </View>
