...
import { ArrowLeft, Calendar, Clock, User, CreditCard as Edit3, Trash2, Copy, Play, Pause, CircleCheck as CheckCircle, X, Save, MoveHorizontal as MoreHorizontal, Target, Activity } from 'lucide-react-native';
import { useColorScheme, getColors } from '@/hooks/useColorScheme';
import { router, useLocalSearchParams } from 'expo-router';
import { WorkoutPlan, Client, DayOfWeek } from '@/types/workout';
import { getPlan, getClient, getTemplates } from '@/utils/storage';
...
export default function PlanDetailsScreen() {
  ...
  const loadPlanDetails = async () => {
    try {
      const planData = await getPlan(id as string);
      if (planData) {
        setPlan(planData);
  ...
  const handleStartWorkout = (day: DayOfWeek) => {
    ...
    if (templateId) {
      Alert.alert('Start Workout', `Starting ${getTemplateName(templateId)} workout for ${day}`);
    }
  };
